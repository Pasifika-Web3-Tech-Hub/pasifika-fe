...
export default function CirclePaymentInfo() {
  ...
  const [circlePaymentId, setCirclePaymentId] = useState('');
  ...
  useEffect(() => {
    const initContract = async () => {
      try {
        ...
        const contractAddress = PasifikaFiatBridgeData.address; // "0x4D2FECe0a2D32685d364705Ae56F46eDD1144461"
        ...
        console.error('Error initializing Circle payment:', err);
  ...
  const handleCirclePaymentIdChange = (e) => {
    setCirclePaymentId(e.target.value);
  };
  
  const initiateCirclePayment = async () => {
    ...
    try {
      ...
      const paymentId = circlePaymentId || `circle-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
      setCirclePaymentId(paymentId);
      
      const amountInWei = ethers.parseUnits(usdcAmount, 6); // USDC has 6 decimals
      
      const tx = await contract.recordCirclePayment(
        paymentId,
        amountInWei,
        selectedCurrency,
        recipient
      ...
      console.error('Error initiating Circle payment:', err);
  ...
  const verifyCirclePayment = async () => {
    if (!contract || !signer || !circlePaymentId) {
    ...
    try {
      ...
      const tx = await contract.verifyCirclePayment(circlePaymentId);
  ...
  return (
    <div className="fiat-bridge-container">
      {isLoading ? (
        <p>Loading Circle payment data...</p>
      ) : (
        <div>
          {error && <div className="error">{error}</div>}
          
          <div className="fiat-bridge-section">
            <h4>Supported Currencies (Circle)</h4>
          ...
          <div className="fiat-bridge-section">
            ...
            <div className="payment-form">
              ...
              <div className="form-group">
                <label>Circle Payment ID:</label>
                <input 
                  type="text" 
                  value={circlePaymentId} 
                  onChange={handleCirclePaymentIdChange} 
                  placeholder="Enter Circle payment ID for verification"
                />
              </div>
              
              <div className="payment-buttons">
                <button 
                  onClick={initiateCirclePayment} 
              ...
              <div className="component-action">
                <button 
                  onClick={verifyCirclePayment}
                  disabled={processingPayment || !circlePaymentId || !walletConnected}
                  className="primary-button command-button"
                >
                  <span className="button-icon">💳</span>
                  <span className="button-text">Verify Circle Payment</span>
